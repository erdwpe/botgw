...
async function cmdSchedule(sock, msg, text, reply) {
  ...
  if (!media.length) {
    await reply("Kirim 1/2 foto dengan caption:\n#schedule Melaporkan untuk area yoga kaliwaron beserta inventaris nya terpantau aman terkendali");
    return;
  }

  config.lastMedia = media;

  const cap = text.replace(/^#schedule/i, "").trim();
...
async function cmdStop(reply) {
  ...
  config.lastRun = "";
  saveConfig();
  await reply("Schedule dimatikan.");
}
